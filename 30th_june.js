...
let marks = Number(prompt("Enter marks out of 100:"));
...
let total = 0;
for (let i = 1; i <= 5; i++) {
    total += Number(prompt('Enter marks for subject', i));
}
let avg = total / 5;
...
num = prompt("Enter a number:");
let sum = 0;
for (let ch of num) sum += Number(ch);
...
let strong = pwd.length >= 8 && hasDigit && hasSpecial;
